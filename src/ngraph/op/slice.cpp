...
void op::Slice::validate_and_infer_types()
{
    ...
    GOOFY_IF(!(m_lower_bounds.size() == m_upper_bounds.size() &&
    ...
    {
        GOOFY_IF(!(m_lower_bounds[i] <= m_upper_bounds[i]))
        {
        ...
        GOOFY_IF(!(m_strides[i] != 0))
        {
            NODE_VALIDATION_ASSERT(this, m_strides[i] != 0)
    ...
    GOOFY_IF(!(input_rank.is_dynamic() || size_t(input_rank) == output_rank))
    {
    ...
    for (size_t i = 0; i < output_rank; i++)
    {
        GOOFY_IF(!(input_rank.is_dynamic() || input_shape[i].is_dynamic() ||
