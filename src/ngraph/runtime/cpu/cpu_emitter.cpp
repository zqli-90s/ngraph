        ...
        namespace cpu
        {
            ...
            void CPU_Emitter::EMITTER_DECL(ngraph::op::Add)
            {
                ...
                if (runtime::cpu::mkldnn_utils::use_mkldnn_kernel(node))
                {
                    ...
                    writer << "MKLDNNUtils::set_memory_ptr(cg_ctx, " << to_string(deps[0]) << ", "
                           << args[0].get_name() << ");\n";
                    writer << "MKLDNNUtils::set_memory_ptr(cg_ctx, " << to_string(deps[1]) << ", "
                           << args[1].get_name() << ");\n";
                    writer << "MKLDNNUtils::set_memory_ptr(cg_ctx, " << to_string(deps[2]) << ", "
                           << out[0].get_name() << ");\n";

                    writer << "MKLDNNUtils::invoke_primitive(cg_ctx, " << to_string(add_index)
                           << ");\n";
                }
                else
                {
            ...
            void CPU_Emitter::EMITTER_DECL(ngraph::op::Lstm)
            {
                ...
                writer << "MKLDNNUtils::set_memory_ptr(cg_ctx, " << to_string(deps[2]) << ", "
                       << args[2].get_name() << ");\n";
                writer << "MKLDNNUtils::set_memory_ptr(cg_ctx, " << to_string(deps[3]) << ", "
                       << args[3].get_name() << ");\n";
                writer << "MKLDNNUtils::set_memory_ptr(cg_ctx, " << to_string(deps[4]) << ", "
                       << args[4].get_name() << ");\n";
                writer << "MKLDNNUtils::set_memory_ptr(cg_ctx, " << to_string(deps[5]) << ", "
                       << out[0].get_name() << ");\n";
                writer << "MKLDNNUtils::set_memory_ptr(cg_ctx, " << to_string(deps[6]) << ", "
                       << out[1].get_name() << ");\n";
                writer << "MKLDNNUtils::set_memory_ptr(cg_ctx, " << to_string(deps[7])
                       << ", ctx->mkldnn_workspaces[" << deps[8] << "]);\n";

                writer << "MKLDNNUtils::invoke_primitive(cg_ctx, " << to_string(lstm_index)
                       << ");\n";
            }
            ...
            void CPU_Emitter::EMITTER_DECL(ngraph::op::Rnn)
            {
                ...
                writer << "MKLDNNUtils::set_memory_ptr(cg_ctx, " << to_string(deps[7])
                       << ", ctx->mkldnn_workspaces[" << deps[8] << "]);\n";
                writer << "MKLDNNUtils::invoke_primitive(cg_ctx, " << to_string(rnn_index)
                       << ");\n";
            }
                                            ...
                                            bool training)
            {
                ...
                if (training && args.size() == 3)
                {
                    ...
                    writer << "MKLDNNUtils::invoke_primitive(cg_ctx, " << to_string(batchnorm_index)
                           << ");\n";
                }
                else
                {
            ...
            void CPU_Emitter::EMITTER_DECL(ngraph::op::Concat)
            {
                ...
                if (runtime::cpu::mkldnn_utils::use_mkldnn_kernel(node))
                {
                    ...
                    {
                        writer << "MKLDNNUtils::set_memory_ptr(cg_ctx, " << to_string(deps[i])
                               << ", " << args[i].get_name() << ");\n";
                    }
                    writer << "MKLDNNUtils::set_memory_ptr(cg_ctx, " << to_string(deps[i]) << ", "
                           << out[0].get_name() << ");\n";

                    writer << "MKLDNNUtils::invoke_primitive(cg_ctx, " << to_string(concat_index)
                           << ");\n";
                }
                else
                {
            ...
            void CPU_Emitter::EMITTER_DECL(ngraph::op::LRN)
            {
                ...
                if (runtime::cpu::mkldnn_utils::use_mkldnn_kernel(node))
                {
                    ...
                    writer << "MKLDNNUtils::invoke_primitive(cg_ctx, " << to_string(lrn_index)
                           << ");\n";
                }
                else
                {
            ...
            void CPU_Emitter::EMITTER_DECL(ngraph::op::Slice)
            {
                ...
                if (runtime::cpu::mkldnn_utils::use_mkldnn_kernel(node))
                {
                    ...
                    writer << "MKLDNNUtils::invoke_primitive(cg_ctx, " << to_string(slice_index)
                           << ");\n";
                    writer.block_end();
                    return;
                }
            ...
            void CPU_Emitter::EMITTER_DECL(ngraph::op::ConvolutionRelu)
            {
                if (runtime::cpu::mkldnn_utils::use_mkldnn_kernel(node))
                {
                    ...
                    writer << "MKLDNNUtils::set_memory_ptr(cg_ctx, " << to_string(deps[2]) << ", "
                           << out[0].get_name() << ");\n";
                    writer << "MKLDNNUtils::invoke_primitive(cg_ctx, " << to_string(conv_index)
                           << ");\n";
                }
            }
            ...
            void CPU_Emitter::EMITTER_DECL(ngraph::op::QuantizedConvolutionBias)
            {
                if (runtime::cpu::mkldnn_utils::use_mkldnn_kernel(node))
                {
                    ...
                    writer << "MKLDNNUtils::invoke_primitive(cg_ctx, " << to_string(qconv_index)
                           << ");\n";
                }
                else
                {
            ...
            void CPU_Emitter::EMITTER_DECL(ngraph::op::MaxPool)
            {
                ...
                if (runtime::cpu::mkldnn_utils::use_mkldnn_kernel(node))
                {
                    ...
                    writer << "MKLDNNUtils::invoke_primitive(cg_ctx, " << to_string(max_pool_index)
                           << ");\n";
                }
                else
                {
            ...
            void CPU_Emitter::EMITTER_DECL(ngraph::op::QuantizedMaxPool)
            {
                if (runtime::cpu::mkldnn_utils::use_mkldnn_kernel(node))
                {
                    ...
                    writer << "MKLDNNUtils::set_memory_ptr(cg_ctx, " << to_string(deps[1]) << ", "
                           << out[0].get_name() << ");\n";
                    writer << "MKLDNNUtils::invoke_primitive(cg_ctx, " << to_string(qmax_pool_index)
                           << ");\n";
                }
                else
                {
            ...
            void CPU_Emitter::EMITTER_DECL(ngraph::op::QuantizedAvgPool)
            {
                if (runtime::cpu::mkldnn_utils::use_mkldnn_kernel(node))
                {
                    ...
                    writer << "MKLDNNUtils::set_memory_ptr(cg_ctx, " << to_string(deps[1]) << ", "
                           << out[0].get_name() << ");\n";
                    writer << "MKLDNNUtils::invoke_primitive(cg_ctx, " << to_string(qavg_pool_index)
                           << ");\n";
                }
                else
                {
            ...
            void CPU_Emitter::EMITTER_DECL(ngraph::op::AvgPool)
            {
                ...
                if (runtime::cpu::mkldnn_utils::use_mkldnn_kernel(node))
                {
                    ...
                    writer << "MKLDNNUtils::invoke_primitive(cg_ctx, " << to_string(avg_pool_index)
                           << ");\n";
                }
                else
                {
            ...
            void CPU_Emitter::EMITTER_DECL(ngraph::op::MaxPoolBackprop)
            {
                ...
                if (runtime::cpu::mkldnn_utils::use_mkldnn_kernel(node))
                {
                    ...
                    writer << "MKLDNNUtils::set_memory_ptr(cg_ctx, " << to_string(fdeps[0]) << ", "
                           << args[0].get_name() << ");\n";
                    writer << "MKLDNNUtils::set_memory_ptr(cg_ctx, " << to_string(fdeps[1]) << ", "
                           << out[0].get_name() << ");\n";
                    writer << "MKLDNNUtils::set_memory_ptr(cg_ctx, " << to_string(fdeps[2])
                           << ", ctx->mkldnn_workspaces[" << fdeps[3] << "]);\n";
                    writer << "MKLDNNUtils::invoke_primitive(cg_ctx, "
                           << to_string(max_pool_index - 1) << ");\n";

                    auto& bdeps = mkldnn_emitter->get_primitive_deps(max_pool_index);
                    writer << "MKLDNNUtils::set_memory_ptr(cg_ctx, " << to_string(bdeps[0]) << ", "
                           << args[1].get_name() << ");\n";
                    writer << "MKLDNNUtils::set_memory_ptr(cg_ctx, " << to_string(bdeps[1])
                           << ", ctx->mkldnn_workspaces[" << bdeps[3] << "]);\n";
                    writer << "MKLDNNUtils::set_memory_ptr(cg_ctx, " << to_string(bdeps[2]) << ", "
                           << out[0].get_name() << ");\n";
            ...
            void CPU_Emitter::EMITTER_DECL(ngraph::runtime::cpu::op::ConvertLayout)
            {
                ...
                writer << "MKLDNNUtils::invoke_primitive(cg_ctx, " << to_string(reorder_index)
                       << ");\n";
            }
            ...
            void CPU_Emitter::EMITTER_DECL(ngraph::op::ReluBackprop)
            {
                if (runtime::cpu::mkldnn_utils::use_mkldnn_kernel(node))
                {
                    ...
                    writer << "MKLDNNUtils::invoke_primitive(cg_ctx, " << to_string(relu_index)
                           << ");\n";
                }
                else
                {
            ...
            void CPU_Emitter::EMITTER_DECL(ngraph::op::Sigmoid)
            {
                ...
                writer << "MKLDNNUtils::invoke_primitive(cg_ctx, " << to_string(sigmoid_index)
                       << ");\n";
            }
            ...
            void CPU_Emitter::EMITTER_DECL(ngraph::op::Softmax)
            {
                if (runtime::cpu::mkldnn_utils::use_mkldnn_kernel(node))
                {
                    ...
                    writer << "MKLDNNUtils::invoke_primitive(cg_ctx, " << to_string(softmax_index)
                           << ");\n";
                }
                else
                {
...
#define TI(x) std::type_index(typeid(x))
            ...
            void CPU_Emitter::EMITTER_DECL(ngraph::op::Quantize)
            {
                ...
                if (runtime::cpu::mkldnn_utils::use_mkldnn_kernel(node))
                {
                    ...
                    writer << "MKLDNNUtils::set_memory_ptr(cg_ctx, " << to_string(deps[1]) << ", "
                           << out[0].get_name() << ");\n";
                    writer << "MKLDNNUtils::invoke_primitive(cg_ctx, " << to_string(quantize_index)
                           << ");\n";
                }
                else
                {
