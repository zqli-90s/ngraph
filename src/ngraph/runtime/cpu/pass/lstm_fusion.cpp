        ...
        [lstm_node_label, xt, weights_h2h, ht_1, weights_i2h, bias1, bias2, ct_1](
            ...
            for (auto& goes : lstm_node->get_outputs().at(0).get_inputs())
            {
                ...
                else if (goe_node->get_n() == 1)
                {
                    ...
                    {
                        for (auto& node_arg : goe_ct_user->get_arguments())
                        {
                            if (node_arg == goe_node)
                            {
                                new_args.push_back(ct_slice);
                            }
                            else
                            {
                        ...
                        ngraph::replace_node(goe_ct_user,
                                             goe_ct_user->copy_with_new_args(new_args));
                        new_args.clear();
                    }
